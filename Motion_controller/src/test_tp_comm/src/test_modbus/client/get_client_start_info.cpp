...
int main(int argc, char** argv)
{
    ...
    unsigned int hash_value = 0x0000084F;
    ...
    ResponseMessageType_Uint64_ModbusClientStartInfo recv_msg;
    unsigned int recv_hash = 0;
    if (!test.decodeResponseMessageType(recv_hash, (void*)&recv_msg, ResponseMessageType_Uint64_ModbusClientStartInfo_fields, buf, buf_size))
    {
    ...
    printf("Reply : msg.data.id = %d\n", recv_msg.data.id);
    printf("Reply : msg.data.name = %s\n", recv_msg.data.name);
    printf("Reply : msg.data.ip = %s\n", recv_msg.data.ip);
    printf("Reply : msg.data.port = %d\n", recv_msg.data.port);
    printf("Reply : msg.data.response_timeout = %d\n", recv_msg.data.response_timeout);
    printf("Reply : msg.data.scan_rate = %d\n", recv_msg.data.scan_rate);
