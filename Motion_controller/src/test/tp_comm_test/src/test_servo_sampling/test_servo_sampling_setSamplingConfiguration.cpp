...
int main(int argc, char* argv[])
{
    if (argc <= 3)
    {
        cout << "3 parameters are needed: cpu_id, interval, max_times" << endl;
        return -1;
    }
    ...
    unsigned int hash_value = 0x0000845E;

    RequestMessageType_Int32_Uint32List msg;
    msg.header.time_stamp = 122;
    msg.property.authority = Comm_Authority_TP_SIMMULATOR;
    msg.data1.data = atoi(argv[1]);
    msg.data2.data_count = 2;
    msg.data2.data[0] = atoi(argv[2]);
    msg.data2.data[1] = atoi(argv[3]);

    if (!test.generateRequestMessageType(hash_value, (void*)&msg, RequestMessageType_Int32_Uint32List_fields, buf, buf_size))
    {
