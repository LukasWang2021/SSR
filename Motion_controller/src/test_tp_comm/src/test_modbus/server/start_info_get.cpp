...
int main()
{
    ...
    unsigned int hash_value = 0x000018AF;
    ...
    ResponseMessageType_Uint64_ModbusServerStartInfo recv_msg;
    unsigned int recv_hash = 0;
    if (!test.decodeResponseMessageType(recv_hash, (void*)&recv_msg, ResponseMessageType_Uint64_ModbusServerStartInfo_fields, buf, buf_size))
    {
    ...
    cout << "Reply : msg.data.ip = " << recv_msg.data.ip << endl;
    cout << "Reply : msg.data.name = " << recv_msg.data.name << endl;
    cout << "Reply : msg.data.response_delay = " << recv_msg.data.response_delay << endl;
