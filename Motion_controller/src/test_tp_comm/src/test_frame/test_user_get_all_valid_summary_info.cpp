...
int main()
{
    ...
    unsigned int hash_value = 0x0001838F;
    ...
    ResponseMessageType_Uint64_UserCoordSummaryList recv_msg;
    unsigned int recv_hash = 0;
    if (!test.decodeResponseMessageType(recv_hash, (void*)&recv_msg, ResponseMessageType_Uint64_UserCoordSummaryList_fields, buf, buf_size))
    {
    ...
    for (int i = 0; i != recv_msg.data.user_coord_summary_count; ++i)
    {
        ...
        cout << "Reply : msg.data.user_coord_summary[" << i << "].comment = " << recv_msg.data.user_coord_summary[i].comment << endl;
        cout << "Reply : msg.data.user_coord_summary[" << i << "].group_id = " << recv_msg.data.user_coord_summary[i].group_id << endl;
    }
