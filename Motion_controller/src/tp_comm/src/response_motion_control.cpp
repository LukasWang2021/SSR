...
void TpComm::handleResponse0x0001487A(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
    if(!encodeResponsePackage(task->hash, ResponseMessageType_Uint64_Double_fields, task->response_data_ptr, send_buffer_size))
    {
        LogProducer::error("comm", "handleResponse: failed to encode response package");// send
...
void TpComm::handleResponse0x00017FF3(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
    if(!encodeResponsePackage(task->hash, ResponseMessageType_Uint64_Double_fields, task->response_data_ptr, send_buffer_size))
    {
        LogProducer::error("comm", "handleResponse: failed to encode response package");// send
...
void TpComm::handleResponse0x0000D37A(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
    if(!encodeResponsePackage(task->hash, ResponseMessageType_Uint64_fields, task->response_data_ptr, send_buffer_size))
    {
        LogProducer::error("comm", "handleResponse: failed to encode response package");// send
...
void TpComm::handleResponse0x00010AF3(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
    if(!encodeResponsePackage(task->hash, ResponseMessageType_Uint64_fields, task->response_data_ptr, send_buffer_size))
    {
        LogProducer::error("comm", "handleResponse: failed to encode response package");// send
...
void TpComm::handleResponse0x00016078(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
    if(!encodeResponsePackage(task->hash, ResponseMessageType_Uint64_DoubleList_fields, task->response_data_ptr, send_buffer_size))
    {
        LogProducer::error("comm", "handleResponse: failed to encode response package");// send
