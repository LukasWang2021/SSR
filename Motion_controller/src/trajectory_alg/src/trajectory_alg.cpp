	...
	if (type == 1) // 加速计算
	{
		for (int i = 0; i < dofn; i++)
		{
			...
			else if (sigmaq[i] != 0 && Ti[i] > 0) // 这一段是根据多轴时间同步后 计算各段的时间系数和多项式系数 并返回结束的位置速度 加速度
			{
				...
				// recompute ta1 with Ti = ta1 + ta2 + ta3 + ta4 and the dq_avg is the constraint factor
				else
				{
					a = (Jm[i] *dq_avg[i] *sigma[i] - Jm[i] *dq0[i] *sigma[i]);
					b = (2 * Jm[i] *qf[i] *sigma[i] - 2 * Jm[i] *q0[i] *sigma[i] - 2 * Jm[i] *Ti[i] *dq_avg[i] *sigma[i]);
					c = dq0[i]*dq0[i] - 2 * dq0[i] *dq_avg[i] + dq_avg[i]*dq_avg[i];
					lamda = b*b - 4 * a*c;
					if (lamda > -minimq && lamda < minimq)
					{
						lamda = 0;
					...
					if (lamda <= 0)
					{
						...
						slamda = b*b - 4 * a*c;
						if (slamda > -minimq && slamda < minimq)
						{
							slamda = 0;
						}
						if (slamda >0)
					...
					else
					{
						...
						if ((newT[0] >0 && newT[1] >0)) // 求出两个正根 取短的时间代入计算
						{
							...
							ta2[i] = (-Jm[i] *sigma[i] *ta1[i]*ta1[i] - dq0[i] + dq_avg[i]) / (Jm[i] *sigma[i] *ta1[i]);
							ddq2[i] = ddq1[i];
							dq2[i] = dq1[i] + ddq1[i] * ta2[i];
							q2[i] = q1[i] + dq1[i] * ta2[i] + 1 / 2.0 * ddq1[i] * pow(ta2[i], 2);

							ta3[i] = ddq1[i] / (sigma[i] * Jm[i]);
							ddq3[i] = ddq2[i] - sigma[i] * Jm[i] * ta3[i];
							dq3[i] = dq2[i] + ddq2[i] * ta3[i] - 1 / 2.0 * sigma[i] * Jm[i] * pow(ta3[i], 2);
							q3[i] = q2[i] + dq2[i] * ta3[i] + 1 / 2.0 * ddq2[i] * pow(ta3[i], 2) - 1 / 6.0 * sigma[i] * Jm[i] * pow(ta3[i], 3);
	...
	else if (type == 2)// slowdown  逐点减速段
	{
		for (int i = 0; i < dofn; i++)
		{
			...
			else if (sigmaq[i] != 0 && Ti[i] < -minimt)// 这一段是根据多轴时间同步后 计算各段的时间系数和多项式系数 并返回结束的位置速度 加速度
			{
				...
				// recompute ta1 with Ti = ta1 + ta2 + ta3 + ta4
				else
				{
					a = (Jm[i] * dq_avg[i] * sigma[i] - Jm[i] * dq0[i] * sigma[i]);
					b = (2 * Jm[i] * q0[i] * sigma[i] - 2 * Jm[i] * qf[i] * sigma[i] - 2 * Jm[i] * Ti[i] * dq_avg[i] * sigma[i]);
					c = dq0[i] * dq0[i] - 2 * dq0[i] * dq_avg[i] + dq_avg[i] * dq_avg[i];
					lamda = b*b - 4 * a*c;
					if (lamda > -minimq && lamda < minimq)
					{
