...
int main(int argc, char* argv[])
{
    if (argc <= 2)
    {
        cout << "2 parameters are needed: cpu_id, file_path" << endl;
        return -1;
    }
    ...
    unsigned int hash_value = 0x00004E41;

    RequestMessageType_Int32_String msg;
    msg.header.time_stamp = 122;
    msg.property.authority = Comm_Authority_TP_SIMMULATOR;
    msg.data1.data = atoi(argv[1]);
    memcpy(msg.data2.data, argv[2], strlen(argv[2]) + 1);

    if (!test.generateRequestMessageType(hash_value, (void*)&msg, RequestMessageType_Int32_String_fields, buf, buf_size))
    {
