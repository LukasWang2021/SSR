...
int main()
{
    ...
    unsigned int hash_value = 0x00010225;

    RequestMessageType_Int32 int32_msg;
    int32_msg.header.time_stamp = 122;
    int32_msg.property.authority = Comm_Authority_TP;
    int method = 0;
    cout << "Please input method that you want to switch (0 - 3): " << endl;
    cin >> method;

    if ((method < 0) && (3 < method))
    {
        cout << "method value error : out of range : 0 - 3" << endl;
        return -1;
    }

    int32_msg.data.data = method;
    cout << "method value is " << method << endl;
