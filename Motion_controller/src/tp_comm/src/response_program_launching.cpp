...
void TpComm::handleResponse0x00011544(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
...
void TpComm::handleResponse0x00010944(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
...
void TpComm::handleResponse0x00016B27(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
    if(!encodeResponsePackage(task->hash, ResponseMessageType_Uint64_fields, task->response_data_ptr, send_buffer_size))
    {
        FST_ERROR("handleResponse: failed to encode response package");// send
