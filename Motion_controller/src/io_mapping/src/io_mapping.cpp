...
int IoMapping::init()
{
	...
	appendSingleIOMapper("data\\io\\io_mapping\\ai_ao_mapping.json", varInfo);
	// DI/DO
	strcpy(varInfo.in , "di") ;  strcpy(varInfo.out , "do") ;
	appendSingleIOMapper("data\\io\\io_mapping\\di_do_mapping.json", varInfo);
	// RI/RO
	strcpy(varInfo.in , "ri") ;  strcpy(varInfo.out , "ro") ;
	appendSingleIOMapper("data\\io\\io_mapping\\ri_ro_mapping.json", varInfo);
	// SI/SO
	strcpy(varInfo.in , "si") ;  strcpy(varInfo.out , "so") ;
	appendSingleIOMapper("data\\io\\io_mapping\\si_so_mapping.json", varInfo);
	// UI/UO
	strcpy(varInfo.in , "ui") ;  strcpy(varInfo.out , "uo") ;
	appendSingleIOMapper("data\\io\\io_mapping\\ui_uo_mapping.json", varInfo);
    print_io_mapper();
#else
...
{
	files_manager_data_path = "";
#ifdef WIN32
    files_manager_data_path = std::string(DATA_PATH);
#else
    fst_parameter::ParamGroup param_;
    param_.loadParamFile("/root/install/share/configuration/machine/programs_path.yaml");
    param_.getParam("file_manager/data", files_manager_data_path);
	printf("forgesight_load_programs_path: %s .\n", files_manager_data_path.c_str());
#endif
...
{
	return (char *)files_manager_data_path.c_str();
}
