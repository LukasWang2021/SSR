...
ErrorCode createTrajectoryFromPath(const ControlPoint &prev_point, ControlPoint &this_point)
{
    ...
    if (this_point.path_point.type == MOTION_LINE || this_point.path_point.type == MOTION_CIRCLE)\
    {
        ...
        {
            double distance = getDistance(forwardKinematics(*((Joint*)prev_point.point.joint)), this_point.path_point.pose);
            this_point.expect_duration = distance / this_point.path_point.source->getCommandVelocity();
        }
    }

    if (this_point.path_point.speed_down == false) {
        // increase speed or constant speed
        double t_max[AXIS_IN_ALGORITHM];
        double t_min[AXIS_IN_ALGORITHM];
        double alpha[AXIS_IN_ALGORITHM] = {g_soft_constraint.j1.max_alpha, g_soft_constraint.j2.max_alpha,
        ...
        caculateDuration(this_point.point, prev_point.point, alpha, t_min, t_max);

        FST_INFO("duration of all joints: %f,%f,%f,%f,%f,%f", t_min[0], t_min[1], t_min[2], t_min[3], t_min[4], t_min[5]);

        // determin duration Time
        double limit_time   = t_min[0];
        size_t limit_index  = 0;
        for (int i = 1; i < AXIS_IN_ALGORITHM; i++) {
            if (t_min[i] > limit_time) {
                limit_index = i;
                limit_time  = t_min[i];
            }
        }

        if (limit_time > this_point.expect_duration * 1.1) {
            this_point.duration = limit_time;
        }
        else {
            if (limit_time > this_point.expect_duration) {
                //this_point.duration = limit_time + (limit_time - this_point.expect_duration) / 2;
                this_point.duration = (limit_time + prev_point.duration) / 2;
            }
            else if (limit_time < this_point.expect_duration) {
                this_point.duration = (this_point.expect_duration + limit_time) / 2;
            }
        }
