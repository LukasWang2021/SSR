/*************************************************************************
	> File Name: motion_control_scara_group.cpp
	> Author: 
	> Mail: 
	> Created Time: 2019年02月28日 星期四 10时56分12秒
 ************************************************************************/
...
ErrorCode ScaraGroup::initGroup(ErrorMonitor *error_monitor_ptr, CoordinateManager *coordinate_manager_ptr, ToolManager *tool_manager_ptr)
{
    vel_ratio_ = 0.0625;
    acc_ratio_ = 0.0625;
    cycle_time_ = 0.001;
        ...
        param.getParam("hard_constraint/lower", lower))
    {
        ...
        {
            hard_constraint_.initConstraint(*(Joint*)(&lower[0]), *(Joint*)(&upper[0]), JOINT_OF_SCARA);
        }
        else
        {
        ...
        param.getParam("firm_constraint/lower", lower))
    {
        if (upper.size() == NUM_OF_JOINT && lower.size() == NUM_OF_JOINT)
        {
            firm_constraint_.initConstraint(*(Joint*)(&lower[0]), *(Joint*)(&upper[0]), JOINT_OF_SCARA);
        }
        else
        {
        ...
        param.getParam("soft_constraint/lower", lower))
    {
        if (upper.size() == NUM_OF_JOINT && lower.size() == NUM_OF_JOINT)
        {
            soft_constraint_.initConstraint(*(Joint*)(&lower[0]), *(Joint*)(&upper[0]), JOINT_OF_SCARA);
        }
        else
        {
    ...
    FST_INFO("Hard/firm/soft constraints from axis-%d to axis-%d:", 0, JOINT_OF_SCARA - 1);
    ...
    // 初始化运动学模块
    FST_INFO("Initializing kinematics of ScaraGroup ...");
    kinematics_ptr_ = new KinematicsToll(path);
    ...
    {
        FST_ERROR("Fail to create dynamics for this Group.");
        return MC_INTERNAL_FAULT;
    }

    if (!dynamics_ptr_->initDynamicAlg(path))
    {
        FST_ERROR("Fail to init dynamics for this Group.");
    ...
    if (param.getParam("trajectory_fifo_size", traj_fifo_size))
    {
        ...
        if (traj_fifo_.initTrajectoryFifo(traj_fifo_size, JOINT_OF_SCARA, log_ptr_, dynamics_ptr_) == SUCCESS)
        {
            FST_INFO("Success.");
        }
        else
    ...
    if (!param.getParam("joint_tracking_accuracy", tracking_accuracy, JOINT_OF_SCARA))
    {
    ...
    // 初始化零位校验模块
    FST_INFO("Initializing calibrator of ScaraGroup ...");
    ErrorCode err = calibrator_.initCalibrator(JOINT_OF_SCARA, &bare_core_, log_ptr_);
    ...
    FST_WARN("ScaraGroup init success.");
    return SUCCESS;
}


size_t ScaraGroup::getNumberOfJoint(void)
{
    return JOINT_OF_SCARA;
}


size_t ScaraGroup::getFIFOLength(void)
{
...
char* ScaraGroup::printDBLine(const int *data, char *buffer, size_t length)
{
    int len = 0;
...
char* ScaraGroup::printDBLine(const double *data, char *buffer, size_t length)
{
    int len = 0;
