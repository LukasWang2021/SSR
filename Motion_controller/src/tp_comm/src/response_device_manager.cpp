...
void TpComm::handleResponse0x0000F574(std::vector<TpRequestResponse>::iterator& task, int& send_buffer_size)
{
    if(!encodeResponsePackage(task->hash, ResponseMessageType_Uint64_DeviceVersionList_fields, task->response_data_ptr, send_buffer_size))
    {
        FST_ERROR("handleResponseGetUserOpMode: failed to encode response package");// send
    }
    if(task->request_data_ptr != NULL)
    ...
    if(task->response_data_ptr != NULL)
    {
        delete (ResponseMessageType_Uint64_DeviceVersionList*)task->response_data_ptr;
    }
}
