...
int main(int argc, char* argv[])
{
    if (argc <= 7)
    {
        cout << "7 parameters are needed: cpu, axis_id, position, velocity, acc, dec, jerk" << endl;
        return -1;
    }
    ...
    unsigned int hash_value = 0x000172C5;

    RequestMessageType_Int32List_Int64 msg;
    msg.header.time_stamp = 122;
    msg.property.authority = Comm_Authority_TP_SIMMULATOR;
    msg.data1.data_count = 6;
    msg.data1.data[0] = atoi(argv[1]);
    msg.data1.data[1] = atoi(argv[2]);
    msg.data2.data = atoi(argv[3]);
    msg.data1.data[2] = atoi(argv[4]);
    msg.data1.data[3] = atoi(argv[5]);
    msg.data1.data[4] = atoi(argv[6]);
    msg.data1.data[5] = atoi(argv[7]);

    if (!test.generateRequestMessageType(hash_value, (void*)&msg, RequestMessageType_Int32List_Int64_fields, buf, buf_size))
    {
