...
// Summary: Search the index of table. 
//------------------------------------------------------------
...
{ 
    ...
    {
        printf("\nError in SearchIndex(): Wrong handle!\n");
        return result;
    }
...
int readWriteSharedMemByIndex(const int handle, void *structure, int index, int access_type)
{
    if((handle < 0 )||(handle >= HANDLE_TABLE_LEN))
    {
        printf("\nError in readWriteSharedMemByIndex(): Wrong handle!\n");
        return 0;
    }
    ...
    table = handleTable[handle].table;
...
{   
    if((handle < 0 )||(handle >= HANDLE_TABLE_LEN))
    {
        printf("\nError in readWriteSharedMem(): Wrong handle!\n");
        return 0;
    }
    if((access_type != MEM_WRITE) && (access_type != MEM_READ))
    {
