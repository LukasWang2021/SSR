...
FocDevice::FocDevice():BaseDevice(DEVICE_TYPE_FOC)
{
    //init(1);
}
...
FocDevice::~FocDevice()
{
    closeDevice(foc_fio_device_);
}

bool FocDevice::init(bool is_real)
{
    is_real_ = is_real;
    foc_fio_device_ = openDevice(FOC_DEVICE_PATH, GrindCmdRegsAddr, FOC_CH1_SIZE);
    if(foc_fio_device_ == NULL)
    {
        return false;
    }
    else
    {
        fio_hw_ptr = (FioHw * )foc_fio_device_->device_ptr;
...
int FocDevice::FioSendCmdPack(uint32_t cmd, uint32_t val)
{
    int wait_cnt=0;
...
int FocDevice::FioRecvReplyPack(uint32_t *pktid_status_cmd, uint32_t *val)
{
    int wait_cnt=0;
    uint32_t t_pktId;
    uint32_t t_opcode;
...
bool FocDevice::getIsReal()
{
    return is_real_;
}
void FocDevice::heartBeatBreak()
{
    is_real_ = false;
}


void FocDevice::FioHeartBeatLoopQuery()
{
    int res = 0;
...
ErrorCode FocDevice::updateStatus(void)
{
