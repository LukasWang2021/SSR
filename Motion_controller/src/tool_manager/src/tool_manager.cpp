...
{   
    ...
    {
        LogProducer::error("Tool","Failed to load ToolManager component parameters");
    ...
    tool_info_file_path_modified_ = std::string(TOOL_DIR_MODIFIED);
    tool_info_file_path_modified_ += param_ptr_->tool_info_file_name_;

    if (!copyFile(tool_info_file_path_.c_str(), tool_info_file_path_modified_.c_str()))
    {
        LogProducer::error("Tool","Failed to backup file");
        return TOOL_MANAGER_LOAD_PARAM_FAILED;
    }

    if(!readAllToolInfoFromYaml(param_ptr_->max_number_of_tools_))
    {
        LogProducer::error("Tool","Failed to load tool info");
...
ErrorCode ToolManager::addTool(ToolInfo& info)
{
    ...
    {
        LogProducer::error("Tool","Failed to add tool %d, invalid tool id", info.id);
    ...
    {
        LogProducer::error("Tool","Failed to add tool %d, write yaml failed", info.id);
...
ErrorCode ToolManager::deleteTool(int id)
{
    if(id >= static_cast<int>(tool_set_.size())
        || id == 0)
    {
        LogProducer::error("Tool","Failed to delete tool %d, invalid tool id", id);
        return TOOL_MANAGER_INVALID_ARG;
    }
        
    tool_set_[id].is_valid = false;
    tool_set_[id].name = "";
    tool_set_[id].comment = "";
    tool_set_[id].group_id = -1;
    memset(&tool_set_[id].data, 0, sizeof(PoseEuler));
    if(!writeToolInfoToYaml(tool_set_[id]))
    {
        LogProducer::error("Tool","Failed to delete tool %d, write yaml failed", id);
        return TOOL_MANAGER_TOOLINFO_FILE_WRITE_FAILED;
    }
    return SUCCESS;
}
...
ErrorCode ToolManager::updateTool(ToolInfo& info)
{
    if(info.id >= static_cast<int>(tool_set_.size())
        ...
        || !tool_set_[info.id].is_valid)
    {
        LogProducer::error("Tool","Failed to update tool %d, invalid tool id", info.id);
        return TOOL_MANAGER_INVALID_ARG;
    }
    ...
    if(!writeToolInfoToYaml(tool_set_[info.id]))
    {
        LogProducer::error("Tool","Failed to update tool %d, write yaml failed", info.id);
        return TOOL_MANAGER_TOOLINFO_FILE_WRITE_FAILED;
    }
    return SUCCESS;
}
...
ErrorCode ToolManager::moveTool(int expect_id, int original_id)
{
    ...
    {
        LogProducer::error("Tool","Failed to move tool %d to %d, invalid tool id", expect_id, original_id);
        return TOOL_MANAGER_INVALID_ARG;
    }
    ...
    tool_set_[original_id].name = "";
    tool_set_[original_id].comment = "";
    ...
    {
        LogProducer::error("Tool","Failed to move tool %d to %d, write yaml failed", expect_id, original_id);
        return TOOL_MANAGER_TOOLINFO_FILE_WRITE_FAILED;
    }
    return SUCCESS;
}
...
ErrorCode ToolManager::getToolInfoById(int id, ToolInfo& info)
{
    if(id >= static_cast<int>(tool_set_.size())
        || id <= 0)
    {
        LogProducer::error("Tool","Failed to get tool %d information, invalid tool id", id);
        return TOOL_MANAGER_INVALID_ARG;
    }
...
bool ToolManager::readAllToolInfoFromYaml(int number_of_tools)
{
    ...
    if (tool_info_yaml_help_.loadParamFile(tool_info_file_path_modified_.c_str()))
    {
    ...
    {
        LogProducer::error("Tool","Failed to read tool info from yaml: %s", tool_info_file_path_modified_.c_str());
	    return false;
    }
}
...
bool ToolManager::writeToolInfoToYaml(ToolInfo& info)
{
    ...
    if(tool_info_yaml_help_.dumpParamFile(tool_info_file_path_modified_.c_str()))
    {
        return true;
    }
    else
    {
        LogProducer::error("Tool","Failed to write tool info to yaml: %s", tool_info_file_path_modified_.c_str());
        return false;
    }
}
