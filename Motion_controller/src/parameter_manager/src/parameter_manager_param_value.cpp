...
static const double FLOATING_PRECISION = 0.0000000000001;
...
{
    string s;
    if (offset >= int(xml.length())) return s;

    int pos = offset;
    const char *cp = xml.c_str() + pos;
    while (*cp && isspace(*cp)) { ++cp; ++pos; }

    if (*cp != '<') return s;

    do { s += *cp; ++pos; } while (*cp++ != '>' && *cp != 0);
    offset = pos;
    return s;
}
...
{
    if (offset >= int(xml.length())) return string();
    
    size_t istart = xml.find(tag, offset);
    if (istart == string::npos) return string();

    istart += strlen(tag);
    string etag = "</";
    etag += tag + 1;
    size_t iend = xml.find(etag, istart);
    if (iend == string::npos) return string();
...
bool nextTagIs(const char* tag, const string &xml, int &offset)
{
    ...
    if (*cp && (strncmp(cp, tag, len) == 0)) { offset += nc + len; return true; }
    return false;
}
...
bool ParamValue::operator==(const ParamValue &other) const
{
    ...
    switch (type_)
    {
        ...
        case TYPEDOUBLE:    return fabs(value_.asDouble - other.value_.asDouble) < FLOATING_PRECISION;
        ...
        {
            if (value_.asStruct->size() != other.value_.asStruct->size()) return false;
...
bool ParamValue::isTypeDoubleOrTypeInt()
{
    if (type_ != TYPEDOUBLE && type_ != TYPEINT)
    {
        throw ParamException("Type error", PARAM_TYPE_ERROR);
    }

    return type_ == TYPEDOUBLE;
}
...
void ParamValue::assertArray(int size)
{
    ...
    {
        if (int(value_.asArray->size()) < size) value_.asArray->resize(size);
    }
    else
    {
        throw ParamException("Type error: expected an array", PARAM_TYPE_ERROR);
...
bool ParamValue::fromXml(const string &valueXml, int &offset)
{
    ...
    invalidParam();
    if (!nextTagIs(VALUE_TAG, valueXml, offset)) return false;       // Not a value, offset not updated
...
bool ParamValue::boolFromXml(const string &value_xml, int &offset)
{
    ...
    if (value_end == value_start || (ivalue != 0 && ivalue != 1)) return false;
...
bool ParamValue::stringFromXml(const string &value_xml, int &offset)
{
    ...
    if (value_end == string::npos) return false;     // No end tag;
...
{
    if (!nextTagIs(DATA_TAG, value_xml, offset)) return false;

    type_ = TYPEARRAY;
    value_.asArray = new ValueArray;
    ParamValue v;
    while (v.fromXml(value_xml, offset)) value_.asArray->push_back(v);       // copy...
...
bool ParamValue::structFromXml(const string &value_xml, int &offset)
{
    ...
    while (nextTagIs(MEMBER_TAG, value_xml, offset))
    {
        ...
        if (!v.isValid()) { invalidParam(); return false; }
...
string ParamValue::structToXml() const
{
    ...
    for (ValueStruct::const_iterator it = value_.asStruct->begin(); it != value_.asStruct->end(); ++it)
    {
        xml += MEMBER_TAG;
        xml += NAME_TAG;
        xml += it->first;
...
std::ostream& ParamValue::write(std::ostream &os) const
{
    switch (type_)
    {
        ...
        case TYPESTRUCT:
        {
            ...
            for (ValueStruct::const_iterator it = value_.asStruct->begin(); it != value_.asStruct->end(); ++it)
            {
                if (it != value_.asStruct->begin()) os << ',';
                os << it->first << ':';
                it->second.write(os);
            }
