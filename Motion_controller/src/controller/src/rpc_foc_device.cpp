...
void ControllerRpc::handleRpc0x0000175B(void* request_data_ptr, void* response_data_ptr)
{
    ...
    if(rq_data_ptr->data.data_count == 2)
    {
        ...
        res = foc_device_ptr_->FioSendCmdPack(t_cmd, t_val);
        if(res == 0)
        {
            res  = foc_device_ptr_->FioRecvReplyPack(&rep_dat1,&rep_dat2);
            if(res==0)
            {
                rs_data_ptr->data.data_count = 2;
                rs_data_ptr->data.data[0] = rep_dat1;
                rs_data_ptr->data.data[1] = rep_dat2;
                LogProducer::info("rpc", "/rpc/foc_device/sendFioCmdPackt success  dat1=0x%x, dat2=0x%x", rs_data_ptr->data.data[0], rs_data_ptr->data.data[1]);
            }
            else
            {
    ...
    else
    {
        ...
        LogProducer::error("rpc", "/rpc/foc_device/sendFioCmdPack failed.");
    }
}
