...
int main()
{
    ...
    unsigned int hash_value = 0x00015930;

    RequestMessageType_String send_msg;
    send_msg.header.time_stamp = 122;
    send_msg.property.authority = Comm_Authority_TP;
    cout << "Please input line id: " << endl;
    char line_id[1024] = "";
    cin >> line_id;
    memcpy(send_msg.data.data, line_id, sizeof(line_id));

    if (!test.generateRequestMessageType(hash_value, (void*)&send_msg, RequestMessageType_String_fields, buf, buf_size))
    {
