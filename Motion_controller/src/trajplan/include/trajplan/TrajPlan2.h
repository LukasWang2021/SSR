...
namespace fst_controller
{
    ...
    private:
        ...
        //规划和插值
        int Smt_arc(double P0[3], double P1[3], double P2[3],
...
//        int Polyn3_7seg_t(double d, double v0, double v1, double t, double a, vector<double> &di);
        ...
        int Interpolation_mode_new(double d, double v0, double v_obj, double &v_cnt1, double a, double T[8], double P[7][4]);
        int Interpolation_mode_t_new(double d, double v0, double v1, double t, double a, double T[8], double P[7][4]);
        int Interp_pandv(double P0[3], double V0[3], double P1[3], double V1[3], vector<ElementSmooth> &E);
        ...
        int MoveJ(const JointPoint &J0, const JointPoint &J1, double v_percentage, vector<ElementSmooth> &E);
        int Circle(const Pose &P0, double &v0,
                   const Pose &P1, const Pose &P2, double vc, int &cnt, 
                   double Time[8], double PolynM[7][4]);
        int OrientSmooth(const vector<Quaternion> &Q, double t_pos[3], double Time[8], double PolynM[7][4]);
    ...
    private:
        ...
        //Transitive parameters for L
        double DDX[3], PST_linestart[3];  //unit line direction vector, line start point
        double TimeL[8], PolynML[7][4];   //Time vector of L, Polynominal parameters of L
        double TimeU[8], PolynMU[7][4];   //Time vector of U, Polynominal parameters of U
        double t_line, t_arc;             //time of line (unrounded), time of arc (or transit, rounded)
        int n_time_line, n_time_arc;      //total vector length of line and arc
        Arc arc;                          //transit arc information
        orienttrans orient;               //Orient info for squad

        vector<ElementSmooth> ThreeElementsxyz;
        double TriEleSta[3];

        //Transitive parameters for J
        vector<ElementSmooth> SixElementsJ1to6;
        JointValues Joint_start,Joint_end;
        int n_time_joints;
   
        // Transitive parameters for C
        double TimeC[8], PolynMC[7][4];
        double t_circle;
        int n_time_circle;
