...
void TpComm::handleRequest0x00005564(int recv_bytes)
{
    // create object for request and response package
    RequestMessageType_Void* request_data_ptr = new RequestMessageType_Void;
    if(request_data_ptr == NULL)
    ...
    ResponseMessageType_Uint64_ModbusClientSummaryStartInfoList* response_data_ptr = new ResponseMessageType_Uint64_ModbusClientSummaryStartInfoList;
    if(response_data_ptr == NULL)
    {
        ErrorMonitor::instance()->add(TP_COMM_MEMORY_OPERATION_FAILED);
        FST_ERROR("Can't allocate memory for response_data");
    ...
    handleRequestPackage(0x00005564, (void*)request_data_ptr, (void*)response_data_ptr, 
        recv_bytes, RequestMessageType_Void_fields, -1);
}
