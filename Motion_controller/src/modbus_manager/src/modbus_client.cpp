...
ErrorCode ModbusClient::setEnableStatus(bool status)
{
    ...
    if (!status) //status = false; is_enable = true
    {
        if (is_running_)
        {
            return MODBUS_CLIENT_NOT_RUNNING;
        }
    ...
    //else status = true; is_enable = false
...
bool ModbusClient::checkConfigParamValid()
{
    if (config_param_.start_info.name.length() == 0
        ...
        || config_param_.start_info.scan_rate < 0
        || config_param_.start_info.response_timeout < 0
        || config_param_.reg_info.coil.addr < 1
        || config_param_.reg_info.coil.addr + config_param_.reg_info.coil.max_nb -1 
            < config_param_.reg_info.coil.addr
        || config_param_.reg_info.discrepte_input.addr < 1
        || config_param_.reg_info.discrepte_input.addr + config_param_.reg_info.discrepte_input.max_nb -1 
            < config_param_.reg_info.discrepte_input.addr
        || config_param_.reg_info.input_reg.addr < 1
        || config_param_.reg_info.input_reg.addr + config_param_.reg_info.input_reg.max_nb -1 
            < config_param_.reg_info.input_reg.addr
        || config_param_.reg_info.holding_reg.addr < 1
        || config_param_.reg_info.holding_reg.addr + config_param_.reg_info.holding_reg.max_nb -1 
            <  config_param_.reg_info.holding_reg.addr)
    {
        return false;
    }
...
ErrorCode ModbusClient::setStartInfo(ModbusClientStartInfo start_info)
{
    ...
    config_param_.start_info = start_info;
    return SUCCESS;
}
...
ErrorCode ModbusClient::setRegInfo(ModbusClientRegInfo reg_info)
{
    ...
    if (reg_info.coil.addr < 1
        || reg_info.coil.addr + reg_info.coil.max_nb -1 
            < reg_info.coil.addr
        || reg_info.discrepte_input.addr < 1
        || reg_info.discrepte_input.addr + reg_info.discrepte_input.max_nb -1 
            < reg_info.discrepte_input.addr
        || reg_info.input_reg.addr < 1
        || reg_info.input_reg.addr + reg_info.input_reg.max_nb -1 
            < reg_info.input_reg.addr
        || reg_info.holding_reg.addr < 1
        || reg_info.holding_reg.addr + reg_info.holding_reg.max_nb -1 
            <  reg_info.holding_reg.addr)
    {
        return MODBUS_CLIENT_INVALID_ARG;
    }
...
{
    if (ctrl_state_ == MODBUS_CLIENT_CTRL_CONNECTED)
    {
...
ErrorCode ModbusClient::open()
{
    if (is_running_) return MODBUS_CLIENT_IS_RUNNING;
