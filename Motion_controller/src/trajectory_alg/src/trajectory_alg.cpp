...
ErrorCode calculateParam(double q0[MAXAXES], double dq0[MAXAXES], double ddq0[MAXAXES], double qf[MAXAXES], double maxdq[MAXAXES], double maxddq[MAXAXES], double Jm[MAXAXES], double Ti[MAXAXES],const double old_coeff[MAXAXES][4][4],const double old_Ta[MAXAXES][4], const double old_tqf[MAXAXES], const double old_dqf[MAXAXES],const double old_ddqf[MAXAXES], int type, double Ta[MAXAXES][4], double coeff[MAXAXES][4][4], double *maxT, double tqf[MAXAXES], double dqf[MAXAXES], double ddqf[MAXAXES])
{
    ...
    //定义临时变量
    double ddq_max[MAXAXES], dq_avg[MAXAXES],dq_avg_new[MAXAXES], sigma[MAXAXES]; //最大加速度 平均速度 总时间 jerk符号
    double sigmaq[MAXAXES];
    ...
    if (type == 1) // 加速计算
    {
        for (int i = 0; i < dofn; i++)
        {
            ...
            else if ((qf[i] - q0[i] < minim) && (qf[i] - q0[i]>-minim))
            {
                sigmaq[i] = 1;	//静止 符号取1
            }

            if (maxdq[i] - dq0[i] > minim)
            {
                sigma[i] = 1; //正转  符号取1
            }
            else if (maxdq[i] - dq0[i] < -minim)
            {
                sigma[i] = -1; //反转 符号取 - 1
            }
            else if ((maxdq[i] - dq0[i] < minim) && (maxdq[i]-dq0[i]>-minim) && (sigmaq[i]==1))
            {
                sigma[i] = 1;	//静止 符号取1
            }
            else if ((maxdq[i] - dq0[i] < minim) && (maxdq[i] - dq0[i]>-minim) && (sigmaq[i] == -1))
            ...
            if (sigmaq[i] != 0 && Ti[i] == -1) // 关节存在运动 且只需计算各轴同步运动时间 进入下面计算
            {
                ...
                if ((dq_avg[i] > (dq3[i]+minim) && sigma[i] == 1) || (dq_avg[i] < (dq3[i]-minim) && sigma[i] == -1) || ((dq_avg[i] - dq3[i]) < minim && (dq_avg[i] - dq3[i]) > -minim))
                {
                    //求ta2
                    ...
                    if ((qf[i] > (q3[i]+minim) && sigma[i] == 1) || (qf[i] < (q3[i]-minim) && sigma[i] == -1))
                    {
                        //速度和加速度都能满足 存在ta4
                    ...
                    else if ((qf[i] - q3[i]) < minim && (qf[i] - q3[i]) > -minim)
                    {
                        //速度加速度都能满足  但没有ta4
                        ta4[i] = 0;
                        tqf[i] = q3[i];
                ...
                else
                {
                    ...
                    double deltadq = 0;
                    if (((dq_avg[i] - dq0[i]) < minim && (dq_avg[i] - dq0[i]) > -minim))
                        deltadq = 0;
                    else
                    ...
                    if ((qf[i] - q0[i] > -minim) && (qf[i] - q0[i] < minim))
                        ta4[i] = 0;
                    else
                    ...
                    {
                        printf("ta4 error\n");
                        return 0x002100B00A40003; //ta4计算错误
                    }
            ...
            else if (sigmaq[i] != 0 && Ti[i] > 0) // 这一段是根据多轴时间同步后 计算各段的时间系数和多项式系数 并返回结束的位置速度 加速度
            {
                ...
                if (((Ti[i]-(old_Ta[i][0]+ old_Ta[i][1]+ old_Ta[i][2]+ old_Ta[i][3]))<minim) && ((Ti[i] - (old_Ta[i][0] + old_Ta[i][1] + old_Ta[i][2] + old_Ta[i][3]))>-minim) )
                {
                ...
                if ((old_Ta[i][0] + old_Ta[i][1] + old_Ta[i][2] + old_Ta[i][3]) <minim && (old_Ta[i][0] + old_Ta[i][1] + old_Ta[i][2] + old_Ta[i][3]) >-minim)
                {
                    Ta[i][0] = 0; Ta[i][1] = 0; Ta[i][2] = 0; Ta[i][3] = Ti[i];
                    tempT[i] = Ta[i][0] + Ta[i][1] + Ta[i][2] + Ta[i][3];
                    coeff[i][0][0] = old_coeff[i][0][0]; coeff[i][0][1] = old_coeff[i][0][1]; coeff[i][0][2] = old_coeff[i][0][2]; coeff[i][0][3] = old_coeff[i][0][3];
                    coeff[i][1][0] = old_coeff[i][1][0]; coeff[i][1][1] = old_coeff[i][1][1]; coeff[i][1][2] = old_coeff[i][1][2]; coeff[i][1][3] = old_coeff[i][1][3];
                    coeff[i][2][0] = old_coeff[i][2][0]; coeff[i][2][1] = old_coeff[i][2][1]; coeff[i][2][2] = old_coeff[i][2][2]; coeff[i][2][3] = old_coeff[i][2][3];
                ...
                if ((sigma[i] == 1 && dq_avg_new[i] > (dq0[i] + minim)) || (sigma[i] == -1 && dq_avg_new[i] < (dq0[i] - minim)))
                {
                    //%time adjust
                    ...
                    else // ta3  lamda大于0
                    {
                        ...
                        if (newT[0]<minim && newT[0]>-minim)
                        {
                            newT[0] = 0;
                        }
                        if (newT[1]<minim && newT[1]>-minim)
                        {
                            newT[1] = 0;
                        }
                        ...
                        if (newT[0] > minim && newT[1] > minim)
                        {
                            ...
                            if ((q3[i] > (qf[i] + minim) && sigma[i] == 1) || (q3[i] < (qf[i] - minim) && sigma[i] == -1))
                            {
                ...
                {
                    if (dq_avg_new[i] - dq0[i] > minim)
                    {
                        sigma[i] = 1; //正转  符号取1
                    }
                    else if (dq_avg_new[i] - dq0[i] < -minim)
                    {
                        sigma[i] = -1; //反转 符号取 - 1
                    }
                    else if (dq_avg_new[i] - dq0[i] > -minim && dq_avg_new[i] - dq0[i] < minim)
                    {
                        sigma[i] = 1;	//静止 符号取1
                    }
            ...
            {
                ta1[i] = 0; ta2[i] = 0; ta3[i] = 0; ta4[i] = 0;
        ...
        for (int i = 0; i < dofn; i++)
        {
            ...
            if (sigmaq[i] != 0 && Ti[i] == 1) // 关节存在运动 且只需计算各轴同步运动时间 进入下面计算
            {
                ...
                if ((dq_avg[i] > (dq3[i]+minim) && sigma[i] == 1) || (dq_avg[i] < (dq3[i]-minim) && sigma[i] == -1) || ((dq_avg[i] - dq3[i]) < minim && (dq_avg[i] - dq3[i]) > -minim))
                {
                    ...
                    if ((q0[i] < (q3[i]-minim) && sigma[i] == 1) || (q0[i] >(q3[i]+minim) && sigma[i] == -1))
                    {
                        //速度和加速度都能满足 存在ta4
                    ...
                    else if ((q0[i] - q3[i]) < minim && (q0[i] - q3[i]) > -minim)
                    {
                        //速度加速度都能满足  但没有ta4
                        ta4[i] = 0;
                        tqf[i] = q3[i];
                ...
                if (((sigma[i] == 1 && dq_avg_new[i] > dq0[i] + minim) || (sigma[i] == -1 && dq_avg_new[i] < dq0[i] - minim)))
                {
                    ...
                    else // ta3  lamda大于0
                    {
                        ...
                        if (newT[0] < -minim && newT[1] < -minim)
                        {
                            ...
                            if ((q3[i] < (q0[i] - minim) && sigma[i] == 1) || (q3[i] > (q0[i] + minim) && sigma[i] == -1))
                            {
                       ...
                       fst_mc::TrajSegment(&segment)[NUM_OF_JOINT])
{
    ...
    double odqf[6] = { 0 };
    double oddqf[6] = { 0 };
    ErrorCode ret = calculateParam(J0, dq0, ddq0, J1, dq_avg, ddq_max, jk, Ti,ocoeff,oTa,otqf,odqf,oddqf, 1, Ta, coeff, &maxT, tqf, dqf, ddqf);
    ...
    ret = calculateParam(J0, dq0, ddq0, J1, dq_avg, ddq_max, jk, Ti,ocoeff,oTa,otqf,odqf,oddqf, 1, Ta, coeff, &maxT, tqf, dqf, ddqf);
                        ...
                        fst_mc::TrajSegment(&segment)[NUM_OF_JOINT])
{
    ...
    double odqf[6] = { 0 };
    double oddqf[6] = { 0 };
    ErrorCode ret = calculateParam(J0, dq0, ddq0, J1, dq_avg, ddq_max, jk, Ti,ocoeff,oTa,otqf,odqf,oddqf, 2, Ta, coeff, &maxT, tqf, dqf, ddqf);
    ...
    ret = calculateParam(J0, dq0, ddq0, J1, dq_avg, ddq_max, jk, Ti,ocoeff,oTa,otqf,odqf,oddqf, 2, Ta, coeff, &maxT, tqf, dqf, ddqf);
                            ...
                            TrajSegment(&segment)[NUM_OF_JOINT])
{
    ...
    double odqf[6] = { 0 };
    double oddqf[6] = { 0 };
    ErrorCode ret = calculateParam(J0, dq0, ddq0, J1, dq_avg, ddq_max, jk, Ti,ocoeff,oTa,otqf,odqf,oddqf, 1, Ta, coeff, &maxT, tqf, tdqf, tddqf);
    ...
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            segment[i].duration[j] = 0;
            for (int k = 0; k < 4; k++)
            {
                segment[i].coeff[j][k] = 0;
            }
        }
    }*/
    for (int i = 0; i < 6; i++)
