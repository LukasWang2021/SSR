...
bool HrReg::addReg(void* data_ptr)
{
	...
	HrRegData objHrRegData = * reg_ptr;
		FST_INFO("setHr: id = %d, comment = %s", objHrRegData.id, objHrRegData.comment);
...
bool HrReg::getReg(int id, void* data_ptr)
{
    ...
    {
	    FST_ERROR("HrReg::getReg isGetInputValid failed at %d", id);
        return false;
    }
    ...
    {
	    FST_ERROR("HrReg::getReg getRegList failed at %d", id);
        return false;
    }
    ...
    reg_ptr->id = reg_data.id;
	    FST_INFO("HrReg::getReg getRegList reg_ptr at %d", reg_ptr->id);
...
bool HrReg::setReg(void* data_ptr)
{
	...
	FST_INFO("HrReg::setReg setRegList at %d with %s", reg_data.id, reg_data.is_valid ? "TRUE" : "FALSE");
    if(!setRegList(reg_data))
    {
	    FST_INFO("HrReg::setReg setRegList failed at %d", reg_data.id);
        return false;
    }
    memcpy(&data_list_[reg_data.id], &reg_ptr->value, sizeof(HrValue));
    return writeRegDataToYaml(reg_data, data_list_[reg_data.id]);
...
bool HrReg::isOutOfPosLimit(const HrValue& data)
{
    for(int i = 0; i < MAX_HR_AXIS_NUM; ++i)
    {
        ...
        {
		//	FST_INFO("isOutOfPosLimit::joint_pos[%d]=%f return false ", i, data.joint_pos[i]);
            return true;
        }
    }
    ...
    for(int i = 0; i < MAX_HR_EXT_AXIS_NUM; ++i)
    {
        ...
        {
		//	FST_INFO("isOutOfPosLimit::ext_pos[%d]=%f return false ", i, data.ext_pos[i]);
            return true;
        }
    }
    return false;
