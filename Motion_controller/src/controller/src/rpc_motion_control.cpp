...
void ControllerRpc::handleRpc0x00011B03(void* request_data_ptr, void* response_data_ptr)
{
    ...
    {
        rs_data_ptr->error_code.data = RPC_PARAM_INVALID;
        LogProducer::error("rpc", "/rpc/motion_control/axis_group/calibrateAllZeroPointOffsets input invalid params group_id = %d", group_id);
        return;
    }

    double offsets[NUM_OF_JOINT];
    rs_data_ptr->error_code.data = group_ptr_[group_id]->calibrateOffset(offsets);
    rs_data_ptr->data.data_count = 9;
    memcpy(&rs_data_ptr->data.data[0], offsets, NUM_OF_JOINT*sizeof(double));

    if (rs_data_ptr->error_code.data == SUCCESS)
        LogProducer::info("rpc", "/rpc/motion_control/axis_group/calibrateAllZeroPointOffsets for group[%d] success", group_id);
    else
        LogProducer::error("rpc", "/rpc/motion_control/axis_group/calibrateAllZeroPointOffsets for group[%d] failed. Error = 0x%llx", group_id, rs_data_ptr->error_code.data);
...
void ControllerRpc::handleRpc0x000131D4(void* request_data_ptr, void* response_data_ptr)
{
    ...
    {
        rs_data_ptr->error_code.data = INVALID_PARAMETER;
        LogProducer::error("rpc", "/rpc/motion_control/axis_group/calibrateSingleZeroPointOffset input invalid data.");
        return;
    }
    ...
    double offsets[NUM_OF_JOINT];
    rs_data_ptr->error_code.data = group_ptr_[group_id]->calibrateOffset((size_t)rq_data_ptr->data.data[1], offsets);
    rs_data_ptr->data.data_count = 9;
    memcpy(&rs_data_ptr->data.data[0], offsets, NUM_OF_JOINT*sizeof(double));
 
    if (rs_data_ptr->error_code.data == SUCCESS)
        LogProducer::info("rpc", "/rpc/motion_control/axis_group/calibrateSingleZeroPointOffset for group[%d] success", group_id);
    else
        LogProducer::error("rpc", "/rpc/motion_control/axis_group/calibrateSingleZeroPointOffset for group[%d] failed. Error = 0x%llx", group_id, rs_data_ptr->error_code.data);
...
void ControllerRpc::handleRpc0x00005AE3(void* request_data_ptr, void* response_data_ptr)
{
    ...
    {
        rs_data_ptr->error_code.data = RPC_PARAM_INVALID;
        LogProducer::error("rpc", "/rpc/motion_control/axis_group/calibrateZeroPointOffsets input invalid params group_id = %d", group_id);
        return;
    }
    ...
    if(rq_data_ptr->data2.data_count == 9)
    {
        ...
        double offsets[NUM_OF_JOINT];
        rs_data_ptr->error_code.data = group_ptr_[group_id]->calibrateOffset(&axis_index[0], size, offsets);
        rs_data_ptr->data.data_count = 9;
        memcpy(&rs_data_ptr->data.data[0], offsets, NUM_OF_JOINT*sizeof(double));
    }
    else
    ...
    if (rs_data_ptr->error_code.data == SUCCESS)
        LogProducer::info("rpc", "/rpc/motion_control/axis_group/calibrateZeroPointOffsets for group[%d] success", group_id);
    else
        LogProducer::error("rpc", "/rpc/motion_control/axis_group/calibrateZeroPointOffsets for group[%d] failed. Error = 0x%llx", group_id, rs_data_ptr->error_code.data);
