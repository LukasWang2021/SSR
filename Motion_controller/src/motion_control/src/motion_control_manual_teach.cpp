...
ErrorCode ManualTeach::init(basic_alg::Kinematics *pkinematics, basic_alg::DynamicAlg* pdynamics, Constraint *pcons, const string &config_file)
{
    if (pkinematics && pdynamics && pcons)
    {
...
void ManualTeach::getDirection(ManualDirection *directions)
{
    for (uint32_t j = 0; j < joint_num_; j++)
        directions[j] = motion_direction_[j];
}
...
ErrorCode ManualTeach::setManualStepAxis(const double *steps)
{
    ...
    if (param.loadParamFile(manual_config_file_) && param.setParam("step/axis", data) && param.dumpParamFile(manual_config_file_))
    {
...
ErrorCode ManualTeach::setManualStepPosition(double step)
{
    if (step > MINIMUM_E3 && step < 100)
    {
        ...
        if (param.loadParamFile(manual_config_file_) && param.setParam("step/position", step) && param.dumpParamFile(manual_config_file_))
        {
            step_position_ = step;
            return SUCCESS;
        }
...
ErrorCode ManualTeach::setManualStepOrientation(double step)
{
    if (step > MINIMUM_E6 && step < 1)
    {
        ...
        if (param.loadParamFile(manual_config_file_) && param.setParam("step/orientation", step) && param.dumpParamFile(manual_config_file_))
        {
...
ErrorCode ManualTeach::manualContinuous(const ManualDirection *directions, const Joint &start)
{
    ...
    for (uint32_t j = 0; j < joint_num_; j++)
    {
        ...
        if (directions[j] != STANDING) manual_index = j;
    }
...
ErrorCode ManualTeach::manualContinuousInJoint(const ManualDirection *dir, const Joint &start)
{
    ...
    for (uint32_t j = 0; j < joint_num_; j++)
    {
        ...
        axis_ds_curve_[j].planDSCurve(0, 1, vel, acc, &jerk, 1.0); // FIX ME
...
ErrorCode ManualTeach::manualContinuousInCartesian(const ManualDirection *dir, double time)
{
    for (uint32_t j = 0; j < 6; j++)
    {
        ...
        else
        {
            ...
            if (axis_ds_curve_valid_[j] == true && stop_time < axis_ds_curve_[j].getDuration())
            {
                if (stop_time < MINIMUM_E9)
                {
                    if (j < 3)
                    {
                        cart_end_.point_[j] = cart_start_.point_[j];
                    }
                    else
                    {
                        cart_end_.euler_[j - 3] = cart_start_.euler_[j - 3];
                    }
