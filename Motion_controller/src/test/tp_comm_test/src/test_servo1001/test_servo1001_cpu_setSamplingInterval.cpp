...
int main(int argc, char* argv[])
{
    if (argc <= 2)
    {
        cout << "2 parameters are needed: cpu, sampling_interval" << endl;
        return -1;
    }
    ...
    unsigned int hash_value = 0x00003EEC;

    RequestMessageType_Int32_Uint32 msg;
    msg.header.time_stamp = 122;
    msg.property.authority = Comm_Authority_TP_SIMMULATOR;
    msg.data1.data = atoi(argv[1]);
    msg.data2.data = atoi(argv[2]);

    if (!test.generateRequestMessageType(hash_value, (void*)&msg, RequestMessageType_Int32_Uint32_fields, buf, buf_size))
    {
