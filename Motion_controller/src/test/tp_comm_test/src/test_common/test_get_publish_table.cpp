...
int main()
{
    ...
    unsigned int hash_value = 0x000147A5;
    ...
    ResponseMessageType_Uint64_PublishTable msg;
    unsigned int recv_hash = 0;
    if (!test.decodeResponseMessageType(recv_hash, (void*)&msg, ResponseMessageType_Uint64_PublishTable_fields, buf, buf_size))
    {
	...
	cout << "Reply : msg.error_code = " <<std::hex<< msg.error_code.data << endl;
    cout << "Reply : msg.data.element_count = " <<std::dec<< msg.data.element_count << endl;

    for (unsigned int i = 0; i != msg.data.element_count; ++i)
    {
        cout << "Reply : msg.data.element[" << i << "].hash = 0x" <<std::hex<< msg.data.element[i].hash <<std::dec<< endl;
        cout << "Reply : msg.data.element[" << i << "].path = " << msg.data.element[i].path << endl;
        cout << "Reply : msg.data.element[" << i << "].message_type = " << msg.data.element[i].message_type << endl;
    }
