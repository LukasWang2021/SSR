...
void BaseGroup::doStateMachine_(void)
{
    ...
    // process fio logic - if offline workmode is not in calculation
    if(!offline_to_pause_request_ && !(mc_state == PAUSING_OFFLINE) && !(mc_state == RESUME_OFFLINE) && !(mc_state == STANDBY_TO_OFFLINE))
    {
		...
		case OFFLINE:
        {
            ...
            if(offline_to_pause_request_)
            {
                if(offline_pausemove_ready)
                {
                    ...
                    {
                        LogProducer::error("mc_sm","[MC_OFFLINE] Offline Pause Trajectory planned failed, exit offline mode");
                        clear_request_ = true;
                    }
...
void BaseGroup::handleClearRequest(MotionControlState &mc_state)
{
    ...
    if (clear_request_)
    {
        ...
        else if (mc_state == STANDBY || mc_state == PAUSE || mc_state == MANUAL || mc_state == MANUAL_TO_STANDBY || mc_state == STANDBY_TO_MANUAL)
        {
            ...
            pause_to_auto_request_ = false;
            pthread_mutex_unlock(&planner_list_mutex_);
            LogProducer::warn("mc_sm", "[clear request] reset auto data SUCCESS");
            ...
            // reset offline_planner_'s data, which are pause_traj, resume_traj, etc.
            offline_planner_.reset();

            LogProducer::warn("mc_sm", "[clear request] reset offline data SUCCESS");

            // clear online cache
            pthread_mutex_lock(&online_traj_mutex_);
            online_trajectory_last_point_ = false;
            pthread_mutex_unlock(&online_traj_mutex_);
            LogProducer::warn("mc_sm", "[clear request] reset online data SUCCESS");
            ...
            manual_to_pause_request_ = false;
            pthread_mutex_unlock(&manual_traj_mutex_);
            LogProducer::warn("mc_sm", "[clear request] reset manual data SUCCESS");

            clear_request_ = false;
            stop_barecore_ = false;
            LogProducer::warn("mc_sm","Group cleared.");
        }
        else if(mc_state == STANDBY_TO_OFFLINE || mc_state == OFFLINE || mc_state == PAUSING_OFFLINE || mc_state == PAUSED_OFFLINE
                || mc_state == RESUME_OFFLINE || mc_state == OFFLINE_TO_STANDBY)
        {
            // exit offline motion
            LogProducer::info("mc_sm","Clear group, MC-state = %s", getMontionControlStatusString(mc_state).c_str());
            mc_state_ = STANDBY;
            ...
            LogProducer::info("mc_sm", "[clear request] reset offline data SUCCESS");
