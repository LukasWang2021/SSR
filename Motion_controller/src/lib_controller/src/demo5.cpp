...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("-----------------------------Test 1-------------------------------------------");
    posee1.position.x = 400;
    posee1.position.y = 400;
    posee1.position.z = 400;
    posee1.orientation.a = 0;
    posee1.orientation.b = 0;
    posee1.orientation.c = 3.14159;
    
    posee2.position.x = 400;
    posee2.position.y = -400;
    posee2.position.z = 500;
    ...
    bool res;
    res = group.MoveJ(jnt2,4000,32000, 1,err);
    //res = group.MoveJ(jnt2,4000,32000, 1,err);
    res = group.MoveL(posee1,300,32000, 2,err);
    res = group.MoveL(posee2,300,32000,100, posee3,300,32000,0, 3,err);
    res = group.MoveL(posee3,300,32000,4,err);
    ...
    res = group.MoveJ(jnt1,4000,32000, 1066,err);
    group.convertPathToTrajectory(10000,err);
    ROS_INFO("planned-path:%d, joint_traj:%d", group.getPlannedPathFIFOLength(),group.getJointTrajectoryFIFOLength());
