...
ErrorCode Calibrator::initCalibrator(size_t joint_num, BareCoreInterface *pcore)
{
    ...
    {
        LogProducer::info("mc","initCalibrator: invalid parameter, joint-number = %d, bare-core-ptr = %p", joint_num, pcore);
    ...
    LogProducer::info("mc","Initializing offset calibrator, number-of-joint = %d.", joint_num_);

    if (!nvram_handler_.init())
    {
        LogProducer::error("mc","Fail to init NvRam.");
        return MC_FAIL_IN_INIT;
    }
    ...
    {
        LogProducer::warn("mc","Offset data in nvram is invalid, rebuild offset data");
    ...
    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to read offset state, code = 0x%llx", err);
        return err;
    }

    LogProducer::info("mc","Offset state: %s", printDBLine((int*)offset_stat_, buffer, LOG_TEXT_SIZE));

    err = readOffsetMask(offset_mask_);

    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to read offset mask, code = 0x%llx", err);
        return err;
    }

    LogProducer::info("mc","Offset mask: %s", printDBLine((int*)offset_mask_, buffer, LOG_TEXT_SIZE));
    ...
    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to read offset joint, code = 0x%llx", err);
        return err;
    }

    LogProducer::info("mc","Offset joint: %s", printDBLine(&offset_joint[0], buffer, LOG_TEXT_SIZE));
    ...
    {
        LogProducer::error("mc","Fail to load thresholds config file(base_group.yaml)");
    ...
    {
        LogProducer::error("mc","Fail to read threshold from config file");
        return MC_LOAD_PARAM_FAILED;
    }

    if (data.size() != joint_num_)
    {
        LogProducer::error("mc","Invalid array size of normal threshold, except %d but get %d", joint_num_, data.size());
        return INVALID_PARAMETER;
    }
    
    LogProducer::info("mc","Threshold-normal: %s", printDBLine(&data[0], buffer, LOG_TEXT_SIZE));
    ...
    if (data.size() != joint_num_)
    {
        LogProducer::error("mc","Invalid array size of lost threshold, except %d but get %d", joint_num_, data.size());
        return INVALID_PARAMETER;
    }

    LogProducer::info("mc","Threshold-lost: %s", printDBLine(&data[0], buffer, LOG_TEXT_SIZE));

    for (size_t i = 0; i < joint_num_; i++)
        lost_threshold_[i] = data[i];
    
    // 下发关节数量到裸核
    LogProducer::info("mc","ID of activated-motor-number: 0x%x", 0x0140);
    LogProducer::info("mc","Send activated-motor-number: %d", joint_num_);
    ...
    if (result != SUCCESS)
    {
        LogProducer::error("mc","Fail to send gear-ratio to barecore, code = 0x%llx", result);
        return result;
    }
    
    LogProducer::info("mc","Success.");
    usleep(50 * 1000);
    ...
    {
        LogProducer::error("mc","Loading offset param file(group_gear_ratio.yaml) failed");
        return MC_LOAD_PARAM_FAILED;
    }

    if (gear_ratio_param_.getParam("gear_ratio/id", id) && gear_ratio_param_.getParam("gear_ratio/data", data))
    {
        LogProducer::info("mc","ID of gear ratio: 0x%x", id);
        LogProducer::info("mc","Send gear ratio: %s", printDBLine(&data[0], buffer, LOG_TEXT_SIZE));
    ...
    {
        LogProducer::error("mc","Fail to read gear ratio config file, err=0x%llx", result);
        return MC_LOAD_PARAM_FAILED;
    }
    ...
    {
        LogProducer::error("mc","Loading coupling coefficient param file(group_coupling_coeff.yaml) failed");
        return MC_LOAD_PARAM_FAILED;
    }

    if (coupling_param_.getParam("coupling_coeff/id", id) && coupling_param_.getParam("coupling_coeff/data", data))
    {
        LogProducer::info("mc","ID of coupling coefficient: 0x%x", id);
        LogProducer::info("mc","Send coupling coefficient:");

        for (size_t i = 0; i < joint_num; i++)
        {
            LogProducer::info("mc","  axis-%d: %s", i, printDBLine(&data[0] + 8 * i, buffer, LOG_TEXT_SIZE));
        }
        ...
        {
            LogProducer::error("mc","Fail to send coupling coefficient to barecore, code = 0x%llx", result);
            return result;
        }
    }
    else
    {
        LogProducer::error("mc","Fail to read coupling coefficient config file");
        return MC_LOAD_PARAM_FAILED;
    }
    ...
    {
        LogProducer::error("mc","Loading offset param file(group_offset.yaml) failed");
        return MC_LOAD_PARAM_FAILED;
    }

    if (offset_param_.getParam("zero_offset/id", id) && offset_param_.getParam("zero_offset/data", data))
    {
        LogProducer::info("mc","ID of offset: 0x%x", id);
        LogProducer::info("mc","Send offset: %s", printDBLine(&data[0], buffer, LOG_TEXT_SIZE));
        result = sendConfigData(id, data);
        ...
        if (result == SUCCESS)
        {
            ...
            LogProducer::info("mc","Current joint: %s", printDBLine(&cur_jnt[0], buffer, LOG_TEXT_SIZE));
            LogProducer::info("mc","Success.");
            usleep(50 * 1000);
        }
        else
        {
            LogProducer::error("mc","Fail to send offset to barecore, code = 0x%llx", result);
            return result;
        }
    }
    else
    {
        LogProducer::error("mc","Fail to read zero offset config file");
        return MC_LOAD_PARAM_FAILED;
    }
...
ErrorCode Calibrator::checkOffset(CalibrateState &cali_stat, OffsetState (&offset_stat)[NUM_OF_JOINT])
{
    ...
    uint32_t encoder_state[NUM_OF_JOINT];
    char buffer[LOG_TEXT_SIZE];
    LogProducer::info("mc","Check zero offset.");
    ...
    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to get offset joint from NvRam, code = 0x%llx.", err);
        current_state_ = MOTION_FORBIDDEN;
    ...
    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to get offset state from NvRam, code = 0x%llx.", err);
        current_state_ = MOTION_FORBIDDEN;
        cali_stat = current_state_;
        memcpy(offset_stat, offset_stat_, sizeof(offset_stat));
        return err;
    ...
    {
        LogProducer::error("mc","Fail to get current joint from share mem."); 
        current_state_ = MOTION_FORBIDDEN;
        cali_stat = current_state_;
        memcpy(offset_stat, offset_stat_, sizeof(offset_stat));
    ...
    LogProducer::info("mc","Curr-joint: %s", printDBLine(&cur_jnt[0], buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","Last-joint: %s", printDBLine(&old_jnt[0], buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","Mask-flags: %s", printDBLine((int*)offset_mask_, buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","Nvram-state: %s", printDBLine((int*)nvram_state, buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","encoder_state: %s", printDBLine((int*)encoder_state, buffer, LOG_TEXT_SIZE));
    // for(size_t i = 0; i < joint_num_; i++)
    // {
    //     LogProducer::info("mc","encoder_state 0 bit, axis: %d, %d", i,encoder_state[i]&1);
    //     LogProducer::info("mc","encoder_state 4 bit, axis: %d, %d", i,(encoder_state[i]>>4)&1);
    // }

    // 当前各关节位置和记录文件中各关节位置进行比对
    OffsetState state[NUM_OF_JOINT];
    checkOffset(cur_jnt, old_jnt, state, encoder_state);
    // LogProducer::info("mc","Curr-state: %s", printDBLine((int*)state, buffer, LOG_TEXT_SIZE));
    ...
    // LogProducer::info("mc","Encoder-err: %s", printDBLine(&encoder_err[0], buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","New-state: %s", printDBLine((int*)state, buffer, LOG_TEXT_SIZE));
    ...
    // LogProducer::info("mc","Offset-state: %s", printDBLine((int*)state, buffer, LOG_TEXT_SIZE));

    if (recorder_need_update)
    {
        // 更新记录文件中的零位标志
        LogProducer::info("mc","Update offset state into recorder.");
        err = writeOffsetState(nvram_state);
        if (err != SUCCESS)
        {
            LogProducer::error("mc","Failed, code = 0x%llx", err);
            current_state_ = MOTION_FORBIDDEN;
            cali_stat = current_state_;
            memcpy(offset_stat, offset_stat_, sizeof(offset_stat));
            return err;
    ...
    cali_stat = current_state_;
    memcpy(offset_stat, offset_stat_, sizeof(offset_stat));
    LogProducer::info("mc","Done, calibrate motion-state is %d", current_state_);
    return SUCCESS;
}
...
void Calibrator::checkOffset(Joint curr_jnt, Joint last_jnt, OffsetState *offset_stat, const uint32_t (&encoder_state)[NUM_OF_JOINT])
{
    for (size_t i = 0; i < joint_num_; i++)
    {
        LogProducer::info("mc","Joint %d: encoder battery alarm bit %d, encoder communication alarm bit: %d", i, encoder_state[i] & 0x1, (encoder_state[i] >> 4) & 0x1);
...
{
    //LogProducer::info("mc","Check Calibrate State.");   
    bool limited = false;
    bool forbidden = false;
    ...
    current_state_ = forbidden ? MOTION_FORBIDDEN : (limited ? MOTION_LIMITED : MOTION_NORMAL);
    //LogProducer::info("mc","Done check Calibrate State, calibrate motion-state is %d", current_state_);   
}
...
{
    LogProducer::info("mc","Calibrate offset of all joints.");
...
//        注意只有在机器人处于完全静止的状态下才能进行零位标定，否则标定结果不具有任何意义；
//------------------------------------------------------------------------------
ErrorCode Calibrator::calibrateOffset(uint32_t index, double *new_offset)
{
    LogProducer::info("mc","Calibrate offset of joint %d", index);
...
//        注意只有在机器人处于完全静止的状态下才能进行零位标定，否则标定结果不具有任何意义；
//------------------------------------------------------------------------------
ErrorCode Calibrator::calibrateOffset(const uint32_t *pindex, uint32_t length, double *new_offset)
{
    char buf[64];
    ...
    for (uint32_t i = 0; i < length; i++)
    {
        ...
        {
            LogProducer::error("mc",str.c_str());
            return INVALID_AXIS_ID;
        }
    }

    LogProducer::info("mc",str.c_str());
    Joint cur_joint;
    ServoState servo_state;
    ...
    {
        LogProducer::error("mc","Fail to get current offset, joint or encoder state from Core1");
        return MC_COMMUNICATION_WITH_BARECORE_FAIL;
    }

    for (uint32_t i = 0; i < joint_num_; i++)
    {
        LogProducer::info("mc","Encoder %d: roll is 0x%x, pulse is 0x%x", i, (cur_encoder[i] >> 16) & 0xFFFF, cur_encoder[i] & 0xFFFF);
    }
    ...
    char buffer[LOG_TEXT_SIZE];
    LogProducer::info("mc","Current-offset: %s", printDBLine(&cur_joint[0], buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","Current-joint:  %s", printDBLine(&cur_offset[0], buffer, LOG_TEXT_SIZE));
    ...
    for (uint32_t i = 0; i < length; i++)
    {
        ...
        {
            LogProducer::info("mc","Lost communication to encoder of axis %d, can not calculate new offset", pindex[i]);
        }
        else
        {
    ...
    LogProducer::info("mc","New-offset: %s", printDBLine(new_offset, buffer, LOG_TEXT_SIZE));
    return SUCCESS;
}
...
double Calibrator::calculateOffset(double current_offset, double current_joint, double target_joint)
{
    ...
    LogProducer::info("mc","Current-offset=%.6f, current-joint=%.6f, target-joint=%.6f, new-offset=%.6f", current_offset, current_joint, target_joint, new_offset);
    return new_offset;
}
...
{
    LogProducer::info("mc","Update zero offset to %.6f of joint %d", offset, index);
    ...
    {
        LogProducer::error("mc","Fail to get current joint from core1.");
        return MC_COMMUNICATION_WITH_BARECORE_FAIL;
    }

    if (((encoder_state[index] >> 4) & 0x1) != 0)
    {
        LogProducer::info("mc","Lost communication to encoder of axis %d, can not update offset", index);
        return MC_COMMUNICATION_WITH_ENCODER_FAIL;
    }

    // 更新配置文件
    vector<double> data;
    LogProducer::info("mc","Update config file ...");

    if (!offset_param_.getParam("zero_offset/data", data))
    {
        LogProducer::error("mc","Fail to get offset from config file;");
        return MC_LOAD_PARAM_FAILED;
    }

    char buffer[LOG_TEXT_SIZE];
    LogProducer::info("mc","Old offset: %s", printDBLine(&data[0], buffer, LOG_TEXT_SIZE));
    data[index] = offset;
    LogProducer::info("mc","New offset: %s", printDBLine(&data[0], buffer, LOG_TEXT_SIZE));

    if (!offset_param_.setParam("zero_offset/data", data) || !offset_param_.dumpParamFile(AXIS_GROUP_DIR"group_offset.yaml"))
    {
        LogProducer::error("mc","Fail to save offset");
        return MC_LOAD_PARAM_FAILED;
    }

    // 更新裸核零位
    LogProducer::info("mc","Offset saved, update offset in core1 ...");
    ErrorCode err = sendOffsetToBareCore();

    if (err != SUCCESS)
    {
        LogProducer::error("mc","Cannot update offset in core1, code = 0x%llx", err);
        return err;
    }

    // 此处usleep为了等待裸核的零位生效，考虑是否有更好的实现方式
    usleep(200 * 1000);
    LogProducer::info("mc","Core1 offset updated, update recorder ...");
    ...
    LogProducer::info("mc","Current joint: %s", printDBLine(&cur_joint[0], buffer, LOG_TEXT_SIZE));

    err = readOffsetJoint(old_joint);
    
    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to get last joint from nvram.");
        return err;
    }
    ...
    LogProducer::info("mc","  flag = %s", printDBLine((int*)offset_stat, buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","  mask = %s", printDBLine((int*)offset_mask, buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","  joint = %s", printDBLine(&old_joint[0], buffer, LOG_TEXT_SIZE));

    if (writeOffsetJoint(old_joint) != SUCCESS || writeOffsetState(offset_stat) != SUCCESS || writeOffsetMask(offset_mask) != SUCCESS)
    {
        LogProducer::error("mc","Fail to update recorder in nvram.");
...
ErrorCode Calibrator::saveJoint(void)
{
    ...
    {
        LogProducer::error("mc","Fail to get current joint from share memory");
        return MC_COMMUNICATION_WITH_BARECORE_FAIL;
    }

    for (size_t i = 0; i < joint_num_; i++)
    {
        // LogProducer::info("mc","saveJoint encoder_state , axis: %d, %d", i, encoder_state[i]);
        // LogProducer::info("mc","saveJoint encoder_state 0 bit, axis: %d, %d", i, encoder_state[i]&1);
        // LogProducer::info("mc","saveJoint encoder_state 4 bit, axis: %d, %d", i, (encoder_state[i]>>4)&1);
        // if ((encoder_state[i] & 0x1) != 0)
        // {
            // LogProducer::error("mc","encoder battery unnormal, axis: %d", i);
            // offset_stat_[i] = OFFSET_LOST;
        // }

        if (((encoder_state[i] >> 4) & 0x1) != 0)
        {
            // LogProducer::error("mc","encoder communication unnormal, axis: %d", i);
    ...
    for (size_t i = 0; i < joint_num_; ++i)
    {
        if (((encoder_state[i]) & 0x1) != 0)
        {
            if (!b_log_flag_[i])
            {
                LogProducer::info("mc","encoder battery unnormal axis: %d", i);
                b_log_flag_[i] = true;
            }   
        ...
        else
        {
            if (b_log_flag_[i])
            {
                LogProducer::info("mc","encoder battery recovery axis: %d", i);
                b_log_flag_[i] = false;
            }   
        }
        ...
        if (((encoder_state[i] >> 4) & 0x1) != 0)
        {
            ...
            {
                LogProducer::info("mc","encoder communication unnormal axis: %d", i);
            }           
        ...
        {
            LogProducer::info("mc","encoder communication recovery axis: %d", i);
    ...
    for (size_t i = 0; i < joint_num_; i++)
    {
        if (offset_stat_[i] != OFFSET_NORMAL)
        {
            // LogProducer::error("mc","offset unormal axis: %d",i);
            cur_jnt[i] = read_nvram_jnt[i];
        }
    }
    ...
    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to record joint, code = 0x%llx", err);
        return err;
    }

    // LogProducer::info("mc","write Offset Joint success");

    //char buffer[LOG_TEXT_SIZE];
    //LogProducer::info("mc","Save current joint into record file: %s", printDBLine(&cur_jnt.j1_, buffer, LOG_TEXT_SIZE));
    //LogProducer::info("mc","Success.");
    return SUCCESS;
}
...
ErrorCode Calibrator::maskOffsetLostError(void)
{
    ...
    LogProducer::info("mc","Mask all lost-offset errors.");
    LogProducer::info("mc","Mask flag: %s", printDBLine((int*)offset_mask_, buffer, LOG_TEXT_SIZE));
    LogProducer::info("mc","Offset state: %s", printDBLine((int*)offset_stat_, buffer, LOG_TEXT_SIZE));
    ...
    for (size_t i = 0; i < joint_num_; i++)
    {
        ...
        {
            LogProducer::info("mc","index = %d, lost its offset, we'll try to mask this error.", i);
    ...
    if (!need_save)
    {
        LogProducer::info("mc","No lost-offset error need mask");
        return SUCCESS;
    }

    LogProducer::info("mc","Saving mask flags ...");
    ErrorCode err = writeOffsetMask(mask);

    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to save mask flags into recorder, code = 0x%llx", err);
        return err;
    }
    ...
    LogProducer::info("mc","All lost-offset errors masked.");
    return SUCCESS;
}
...
{
    char buffer[LOG_TEXT_SIZE];
    LogProducer::info("mc","Set offset state, index = %d, state = %d", index, stat);

    if (index >= joint_num_)
    {
        LogProducer::error("mc","Invalid index: %d", index);
        return INVALID_AXIS_ID;
    }
    ...
    LogProducer::info("mc","Offset state: %s", printDBLine((int*)stats, buffer, LOG_TEXT_SIZE));
    stats[index] = stat;
    LogProducer::info("mc","  changed to: %s", printDBLine((int*)stats, buffer, LOG_TEXT_SIZE));
    ...
    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to get last joint from recorder, code = 0x%llx", err);
        return err;
    }

    if (!bare_core_ptr_->getLatestJoint(cur_joint, encoder_state, servo_state))
    {
        LogProducer::error("mc","Fail to get current joint from bare core.");
        return MC_COMMUNICATION_WITH_BARECORE_FAIL;
    }

    if (((encoder_state[index] >> 4) & 1) != 0)
    {
        LogProducer::info("mc","Lost communication to encoder of axis %d, can not set offset state", index);
        return MC_COMMUNICATION_WITH_ENCODER_FAIL;
    }
    ...
    {
        LogProducer::error("mc","Fail to set offset flag into recorder");
        return MC_OPERATE_NVRAM_FAILED;
    }
...
ErrorCode Calibrator::setOffset(size_t index, double offset)
{
    ...
    {
        LogProducer::error("mc","Given index out of range, joint-number is %d", joint_num_);
        return INVALID_AXIS_ID;
    }
}
...
ErrorCode Calibrator::setOffset(const double *offset)
{
    ...
    LogProducer::info("mc","Set offset of all joints, new-offset: %s", printDBLine(offset, buf, LOG_TEXT_SIZE));
...
bool Calibrator::isReferenceAvailable(void)
{
    ...
    if (params.loadParamFile(AXIS_GROUP_DIR"arm_group_offset_reference.yaml"))
    {
        ...
        {
            LogProducer::info("mc","Reference point is %s", (is_available ? "available" : "inavailable"));
            return is_available;
        }
        else
        {
            LogProducer::error("mc","Fail to get item from config file");
            return false;
        }
    }
    else
    {
        LogProducer::error("mc","Fail to load config file(arm_group_offset_reference.yaml)");
        return false;
    }
}
...
{
    LogProducer::info("mc","Delete reference point.");
    base_space::YamlHelp params;
    ...
    if (params.loadParamFile(AXIS_GROUP_DIR"arm_group_offset_reference.yaml"))
    {
        ...
        if (params.getParam("reference_available", is_available))
        {
            if (is_available)
            {
                ...
                {
                    LogProducer::info("mc","Inactive reference point success.");
                    return SUCCESS;
                }
                else
                {
                    LogProducer::error("mc","Error while modifying reference point");
                    return MC_LOAD_PARAM_FAILED;
                }
            }
            else
            {
                LogProducer::info("mc","Reference point is inavailable, nothing to do.");
                return SUCCESS;
            }
        }
        else
...
ErrorCode Calibrator::saveReference(void)
{
    ...
    LogProducer::info("mc","Save referenece point");
    base_space::YamlHelp params;

    if (!params.loadParamFile(AXIS_GROUP_DIR"arm_group_offset_reference.yaml"))
    {
        LogProducer::error("mc","Error while load reference file(arm_group_offset_reference.yaml)");
        return MC_LOAD_PARAM_FAILED;
    }
    ...
    LogProducer::info("mc","  joint:  %s", printDBLine(&ref_joint[0], buffer, LOG_TEXT_SIZE));
    ...
    if (err != SUCCESS)
    {
        LogProducer::error("mc","Fail to get current offset from bare core, code = 0x%llx", err);
        return err;
    }

    LogProducer::info("mc","  offset: %s", printDBLine(&ref_offset[0], buffer, LOG_TEXT_SIZE));
    ...
    {
        LogProducer::error("mc","Fail to get current encoder from bare core");
        return MC_COMMUNICATION_WITH_BARECORE_FAIL;
    }

    LogProducer::info("mc","  encoder: %s", printDBLine(&ref_encoder[0], buffer, LOG_TEXT_SIZE));
    ...
    {
        LogProducer::error("mc","Error while saving reference point");
...
{
    LogProducer::info("mc","Easy calibrate of all joints.");
    size_t indexs[NUM_OF_JOINT] = {0, 1, 2, 3, 4, 5, 6, 7, 8};
...
{
    LogProducer::info("mc","Fsst calibrate index = %d", index);
...
{
    LogProducer::info("mc","Fast calibrate offset");
    ...
    for (size_t i = 0; i < length; i++)
    {
        if (pindex[i] < joint_num_)
        {
            LogProducer::info("mc","  joint index = %d", pindex[i]);
        }
        else
        {
            LogProducer::error("mc","  joint index = %d, index out of range", pindex[i]);
            return INVALID_AXIS_ID;
        }
    }
        ...
        jtac.loadParamFile("share/configuration/machine/jtac.yaml"))
    {
            ...
            jtac.getParam("gear_ratio/data", gear_ratio))
        {
            if (ref_offset.size() == joint_num_ && ref_encoder.size() == joint_num_ && gear_ratio.size() == joint_num_)
            {
                ...
                LogProducer::info("mc","Reference offset: %s", printDBLine(&ref_offset[0], buffer, LOG_TEXT_SIZE));
                LogProducer::info("mc","Reference encoder: %s", printDBLine(&ref_encoder[0], buffer, LOG_TEXT_SIZE));
                LogProducer::info("mc","Gear ratio: %s", printDBLine(&gear_ratio[0], buffer, LOG_TEXT_SIZE));

                if (bare_core_ptr_->getEncoder(cur_encoder))
                {
                    size_t index;
                    LogProducer::info("mc","Current encoder: %s", printDBLine(&cur_encoder[0], buffer, LOG_TEXT_SIZE));
                    ...
                    for (size_t i = 0; i < length; i++)
                    {
                        ...
                        if(((encoder_state[index]>>4)&1) != 0)
                        {
                            LogProducer::info("mc","encoder communication unnormal,axis: %d, do not fast calculate & need save Offset",index+1);
                    ...
                    LogProducer::info("mc","New offset: %s", printDBLine(zero_offset_, buffer, LOG_TEXT_SIZE));
                    return SUCCESS;
                }
                else
                {
                    LogProducer::error("mc","Fail to get current encoders.");
                    return MC_COMMUNICATION_WITH_BARECORE_FAIL;
                }
            }
            else
            {
                if (ref_offset.size() != joint_num_)
                    LogProducer::error("mc","Invalid array size of reference offset, size is %d but %d wanted.", ref_offset.size(), joint_num_);
                else if (ref_encoder.size() != joint_num_)
                    LogProducer::error("mc","Invalid array size of reference encoder, size is %d but %d wanted.", ref_encoder.size(), joint_num_);
                else
                    LogProducer::error("mc","Invalid array size of gear ratio, size is %d but %d wanted.", gear_ratio.size(), joint_num_);
...
{
    LogProducer::info("mc","reset encoder multi-turn value of all joints.");

    if (!bare_core_ptr_->resetEncoderError())
    {
        LogProducer::error("mc","Fail to reset encoder errors.");
        return MC_COMMUNICATION_WITH_BARECORE_FAIL;
    }

    usleep(250 * 1000);

    LogProducer::info("mc","Zero loss flag set");
    for (size_t i = 0; i < joint_num_; i++)
    {
        offset_stat_[i] = OFFSET_LOST;
    }
...
{
    LogProducer::info("mc","Reference-offset = %.6f, reference-encoder = 0x%x, current-encoder = 0x%x, gear-ratio = %.6f",
    ...
    LogProducer::info("mc","Reference-rolls = 0x%x, current-rolls = 0x%x, new-offset = %.6f", ref_rolls, cur_rolls, new_offset);
    return new_offset;
}
