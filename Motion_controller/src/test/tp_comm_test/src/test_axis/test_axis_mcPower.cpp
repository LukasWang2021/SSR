...
int main(int argc, char* argv[])
{
    if (argc <= 2)
    {
        cout << "2 parameters are needed: axis_id, enable" << endl;
        return -1;
    }
    ...
    unsigned int hash_value = 0x000053E2;

    RequestMessageType_Int32_Bool msg;
    msg.header.time_stamp = 122;
    msg.property.authority = Comm_Authority_TP_SIMMULATOR;
    msg.data1.data = atoi(argv[1]);
    msg.data2.data = atoi(argv[2]);

    if (!test.generateRequestMessageType(hash_value, (void*)&msg, RequestMessageType_Int32_Bool_fields, buf, buf_size))
    {
