...
int main()
{
    ...
    unsigned int hash_value = 0x0001104F;
    ...
    ResponseMessageType_Uint64_ToolSummaryList recv_msg;
    unsigned int recv_hash = 0;
    if (!test.decodeResponseMessageType(recv_hash, (void*)&recv_msg, ResponseMessageType_Uint64_ToolSummaryList_fields, buf, buf_size))
    {
    ...
    cout << "Reply : msg.data.tool_summary_info_count = " << recv_msg.data.tool_summary_info_count << endl;
    for (size_t i = 0; i != recv_msg.data.tool_summary_info_count; ++i)
    {
        cout << "Reply : msg.data.tool_summary_info[" << i << "].id = " << recv_msg.data.tool_summary_info[i].id << endl;
        cout << "Reply : msg.data.tool_summary_info[" << i << "].name = " << recv_msg.data.tool_summary_info[i].name << endl;
        cout << "Reply : msg.data.tool_summary_info[" << i << "].comment = " << recv_msg.data.tool_summary_info[i].comment << endl;
        cout << "Reply : msg.data.tool_summary_info[" << i << "].group_id = " << recv_msg.data.tool_summary_info[i].group_id << endl;
    }
