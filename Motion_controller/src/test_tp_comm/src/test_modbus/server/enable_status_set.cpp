...
int main(int argc, char** argv)
{
    ...
    bool connect_status = atoi(argv[1]);
    printf("connect_status = %d\n", connect_status);

    RequestMessageType_Bool msg;
    msg.header.time_stamp = 122;
    msg.property.authority = Comm_Authority_TP;
    msg.data.data = connect_status;

    unsigned int hash_value = 0x00004033;

    if (!test.generateRequestMessageType(hash_value, (void*)&msg, RequestMessageType_Bool_fields, buf, buf_size))
    {
