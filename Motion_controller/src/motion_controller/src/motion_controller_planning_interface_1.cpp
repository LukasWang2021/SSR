...
//------------------------------------------------------------------------------
bool PlanningInterface::setJointConstraint(const JointConstraint &constraint) {
...
bool PlanningInterface::getJointFromPose(const Pose &pose, const Joint &reference,
                                         Joint &joint, double time_interval, ErrorCode &err) {
    computeInverseKinematics(pose, reference, joint, err);
    if (err == SUCCESS) {
        return true;
    }
    else if (err == IK_EXCESSIVE_DISTANCE) {
        if (abs(reference.j1 - joint.j1) < joint_constraint_.j1.max_omega * time_interval &&
            abs(reference.j2 - joint.j2) < joint_constraint_.j2.max_omega * time_interval &&
            abs(reference.j3 - joint.j3) < joint_constraint_.j3.max_omega * time_interval &&
            abs(reference.j4 - joint.j4) < joint_constraint_.j4.max_omega * time_interval &&
            abs(reference.j5 - joint.j5) < joint_constraint_.j5.max_omega * time_interval &&
            abs(reference.j6 - joint.j6) < joint_constraint_.j6.max_omega * time_interval) {
                err = SUCCESS;
                return true;
        }
        else {
...
bool PlanningInterface::addMotionCommand(int id,
                                         ...
                                         ErrorCode &err)
{
    ...
    MoveCommand *new_motion = new MoveCommand(id, planner_, target, next);
    if (new_motion == NULL) {err = MOTION_INTERNAL_FAULT; return false;}
...
bool PlanningInterface::deleteMotionCommand(ErrorCode &err)
{
    if (motion_list_front_ != NULL) {
        // one or more commands in command list
        MoveCommand *command = motion_list_front_;
        if (command != motion_list_back_) {
            // two or more commands in command_list, delete the first one
            motion_list_front_ = command->next;
        ...
        // no command in command list
        return false;
    }
}

bool PlanningInterface::deleteMotionCommand(MoveCommand *command, ErrorCode &err)
{
    if (command != NULL) {
...
bool PlanningInterface::MoveJ2J(JointPoint &jp_start,
                                const Joint &j_target, double v_target, int cnt_target,
                                const Joint &j_next,   double v_next,   int cnt_next,
                                ErrorCode &err) {
...
bool PlanningInterface::MoveJ2L(const JointPoint &jp_start,
                                const Joint &joint_target, double v_target, int cnt_target,
...
bool PlanningInterface::pickPoints(vector<Joint> &points, ErrorCode &err) {
    points.clear();
    if (fifo_length_ <= 0) {err = MOTION_INTERNAL_FAULT; return false;}
...
bool PlanningInterface::replanPauseTrajectory(std::vector<Joint> &trajectory, ErrorCode &err) {
...
bool PlanningInterface::replanRestartTrajectory(std::vector<Joint> &trajectory,
                                                Joint &start_point,
                                                ErrorCode &err) {
