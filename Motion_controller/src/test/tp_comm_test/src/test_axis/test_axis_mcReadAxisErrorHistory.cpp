...
int main(int argc, char* argv[])
{
    ...
    unsigned int hash_value = 0x00018469;
    ...
    ResponseMessageType_Uint64_Uint64List recv_msg;
    unsigned int recv_hash = 0;
    if (!test.decodeResponseMessageType(recv_hash, (void*)&recv_msg, ResponseMessageType_Uint64_Uint64List_fields, buf, buf_size))
    {
    ...
    for (unsigned int i = 0; i < recv_msg.data.data_count; ++i)
        std::cout << "Reply : msg.data.data[" << i << "] = 0x" <<std::hex << recv_msg.data.data[i] << std::endl;
