			   ...
			   cJSON *jsonCart, PoseEuler & cart)
{
	...
	while (child) //  && !fail)
	{
	//	FST_INFO("parsePosesContent: cJSON_Array %s\n", child->string);
		switch ((child->type)&255)
		{
		case cJSON_Number:	
			// FST_INFO("cJSON_Number %f\n", child->valuedouble); 
...
int parseProgramProp(struct thread_control_block * objThreadCntrolBlock, char * data)
{
	...
	while(child)
	{
		...
		case cJSON_Array:	
			{
				FST_INFO("parseDIOMap: cJSON_Array %s\n", child->string);
